    ...
    public class SymbolicRegex<S> : IMatcher, ISerializable
    {
        ...
        int DeltaPlus(string input, int q, out SymbolicRegexNode<S> regex)
        {
            ...
            else
            {
                ...
                for (int i = 0; i < input.Length; i++)
                {
                    ...
                    if (c == 10)
                    {
                        p = DeltaBorder(BorderSymbol.EndLine, q, out _);
                        p = Delta(10, p, out _);
                        p = DeltaBorder(BorderSymbol.StartLine, p, out _);
                    }
                    else
                        p = Delta(c, q, out _);
        ...
        private int FindEndPosition(string input, int i)
        {
            ...
            while (i < k)
            {
                ...
                if (c == 10)
                {
                    p = DeltaBorder(BorderSymbol.EndLine, q, out regex);
                    if (regex.IsNullable)
        ...
        private int FindStartPosition(string input, int i, int match_start_boundary)
        {
            ...
            while (i >= match_start_boundary)
            {
                ...
                else if (i == 0)
                {
                    ...
                    p = DeltaBorder(BorderSymbol.Start, p, out regex);
                    if (regex.isNullable)
                    {
                        last_start = 0;
                        break;
